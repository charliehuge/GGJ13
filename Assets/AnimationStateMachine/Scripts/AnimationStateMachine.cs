...
public class AnimationStateMachine : ExtendedMonoBehaviour {
	...
	void Start () {
		try {
			...
			foreach( AnimationState s in Target.animation ) {
	...
	void OnGUI_PerWeight() {
		...
		foreach( Mixamo.Clip clip in graph.clips ) {
			...
			GUILayout.Label( s.name.ToString() + " - " + Target.animation.IsPlaying( s.name ) + " - " + FormatFloat( s.length ) + " - " + string.Format( "{0:0.00}" , s.weight ) + " - " + s.layer.ToString() + " - " + FormatFloat( s.normalizedSpeed ) , GUILayout.ExpandWidth( true ));
	...
	private RootMotionResult _CalculateRootMotion() {
		if( lastRootMotionResult == null ) {
			...
			foreach (AnimationState aState in Target.animation)
			{
				if( aState.layer == 0 ) {
